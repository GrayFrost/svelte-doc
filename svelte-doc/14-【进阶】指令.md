...
<script lang="ts">
	...
	import { space } from '$lib/_utils/directives';
...
<div use:space={spaceClasses} class={spaceWrapperClasses}>
	<slot />
</div>
```
...
除了use，Svelte还支持其他指令。如在介绍数据与dom引用时的`bind:`，在动画章节介绍的`transition:`、`in:`、`out:`、`animate:`以及在样式章节介绍的`style:`。
